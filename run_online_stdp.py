...
def parse_args():
    parser = OptionParser()
    parser.set_defaults(T=300, K=20, D=-1, W=-1, eta=0.01, alpha=1.0, gamma=1.0, real_gamma=1.0,
    ...
    parser.add_option("--real_gamma", type="float", dest="real_gamma",
                      help="hyperparameter for the noise beta distribution[1.0, 1.0]")
...
def run_online_stdp():
    ...
    if options.init_model_path is None:
        print "%s creating online stdp instance." % getTime()
        ostdp = onlinestdp.online_stdp(options.T, options.K, options.D, options.W,
                                    options.eta, options.alpha, options.gamma, options.real_gamma,
                                    options.kappa, options.tau, options.scale,
                                    options.adding_noise)
        ostdp.init_time_location_parameters(read_time_location(options.data_path))
        cPickle.dump(ostdp, file('%s/init.model' % result_directory, 'wb'), -1)
    else:
        print "%s read online stdp instance from %s." % (getTime(), options.init_model_path)
        ostdp = cPickle.load(file(options.init_model_path, 'rb'))

    if options.new_init:
        ostdp.new_init(c_train)
    ...
    while True:
        ...
        (score, count, unseen_score, unseen_count) = ostdp.process_documents(total_doc_count - batchsize, docs, options.var_converge, unseen_ids)
        ...
        if total_doc_count % options.save_lag == 0:
            ...
            ostdp.save_topics('%s/doc_count-%d.topics' % (result_directory, total_doc_count))
            cPickle.dump(ostdp, file('%s/doc_count-%d.model' % (result_directory, total_doc_count), 'wb'), -1)

            if options.test_data_path is not None:
                print "\t %s working on predictions." % getTime()
                (lda_alpha, lda_beta) = ostdp.hdp_to_lda()
                ...
                for doc in c_test.docs:
                    (likelihood, gamma) = onlinestdp.lda_e_step(doc, lda_alpha, lda_beta)
                    test_score += likelihood
                    (likelihood, count, gamma) = onlinestdp.lda_e_step_split(doc, lda_alpha, lda_beta)
    ...
    ostdp.save_topics('%s/final.topics' % result_directory)
    cPickle.dump(ostdp, file('%s/final.model' % result_directory, 'wb'), -1)
