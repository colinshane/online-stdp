...
def read_time_location(filename):
    c = corpus()
    for line in open(filename):
        d = document()
        line = line.strip()
        info = line.split(" ")[-5:]
        d.time = int(time.mktime(time.strptime(info[0] + " " + info[1], "%Y-%m-%d %H:%M:%S")))
        d.latitude = float(info[-2])
        d.longitude = float(info[-1])
        c.docs.append(d)
    c.num_docs = len(c.docs)
    return c


def read_stream_data_time_location(f, num_docs):
    c = corpus()
    splitexp = re.compile(r'[ :\t]')
    for i in range(num_docs):
        line = f.readline()
