...
class online_hdp:
    ...
    def __init__(self, T, K, D, W, eta, alpha, gamma, kappa, tau, scale=1.0, adding_noise=False):
        ...
        self.m_var_sticks = np.zeros((2, T-1)) # 保存第一层stick的变分参数u,v的更新公式的sigma部分
        ...
        #self.m_lambda = np.random.gamma(100., 1./100., (T, W)) - eta
        self.m_eta = eta
        # the topics Eq[log(P(w_jn=w|phi_k))] batch版本没有加上m_eta
        ...
        # 保存每一步的log(1-rho)
        self.m_r_step = [0]
        self.m_lambda_sum = np.sum(self.m_lambda, axis=1)  # 横着加，加完是K维
    ...
    def process_documents(self, batch_head, docs, var_converge, unseen_ids=[], update=True, opt_o=True):
        ...
        if self.m_adding_noise:  # 默认是false
        ...
        self.m_lambda[:, word_list] *= np.exp(self.m_r[-1] - rw)  # self.mr[-1]表示最新的累加的log(1-rho), 此处更新之前上一轮没有更新的单词的lambda

        # # 累加每个word的eta
        # # 更新公式： lambda[2] = (1-rho2)*(1-rho1)*lambda[0] + (1-rho2)*rho1*eta + rho2*eta
        # # 1. 先对每个单词的累加的log(1-rho)进行分解，得到一个w*t(上次更新经过的轮数)的矩阵
        # rw_step = [self.m_r_step[t + 1 : len(self.m_r_step)] for t in self.m_timestamp[word_list]]
        # # 2. 先反向求累加和然后向右移一位，左端补0
        # rw_step_sum = [np.roll(np.cumsum(np.flipud(self.m_r_step[t + 1 : len(self.m_r_step)])), 1) for t in self.m_timestamp[word_list]]
        # # 3. 然后两者相加
        # for arr_sum, arr_step in zip(rw_step_sum, rw_step):
        #     if len(arr_sum) != 0 and len(arr_step) != 0:
        #         arr_sum[0] = 0
        #         arr_sum += np.flipud(np.log(1 - np.exp(arr_step)))
        # # 4. 计算每个单词的累加eta
        # rw_eta = [np.sum(np.exp(a)) * self.m_eta for a in rw_step_sum]
        # # 5. 将结果更新到lambda中
        # self.m_lambda[:, word_list] += rw_eta
        ...
        Elogsticks_1st = expect_log_sticks(self.m_var_sticks)  # global sticks 和batch版本hdp一样
        ...
        for i, doc in enumerate(docs):
            ...
            doc_score = self.doc_e_step(i + batch_head, doc, ss, Elogsticks_1st, word_list, unique_words, var_converge)
            count += doc.total
            score += doc_score
            if i in unseen_ids:
        ...
        if adding_noise:
            # add noise to the ss
            if(i % 1000 == 0):
                print "%s [batch_head = %d] adding noise at this stage..." % (getTime(), batch_head)
    ...
    # 注意这里没有传入Elogbeta，而是使用保存在模型中的Elogbeta
    ...
    def update_lambda(self, sstats, word_list, opt_o): 
        ...
        # 每一步加上一个eta
        # self.m_lambda[:, word_list] = self.m_lambda[:, word_list] * (1-rhot) + self.m_eta + \
        #      rhot * self.m_D * sstats.m_var_beta_ss / sstats.m_batchsize
        self.m_lambda_sum = (1-rhot) * self.m_lambda_sum + \
            rhot * self.m_D * np.sum(sstats.m_var_beta_ss, axis=1) / sstats.m_batchsize

        # 同上，这里不止加上batch中出现过的word的eta，而是加上了所有的单词的eta
        # self.m_lambda_sum = (1-rhot) * self.m_lambda_sum + (self.m_eta * self.m_W) + \
        #     rhot * self.m_D * np.sum(sstats.m_var_beta_ss, axis=1) / sstats.m_batchsize
        ...
        # 保存每一步的log(1-rho)
        # self.m_r_step.append(np.log(1-rhot))
    ...
    def update_expectations(self):
        ...
        for w in range(self.m_W):
            ...
            # # 累加这个word的eta
            # # 更新公式： lambda[2] = (1-rho2)*(1-rho1)*lambda[0] + (1-rho2)*rho1*eta + rho2*eta
            # # 1. 先对这个单词的累加的log(1-rho)进行分解，得到一个t(上次更新经过的轮数)的数组
            # rw_step = self.m_r_step[self.m_timestamp[w] + 1: len(self.m_r_step)]
            # # 2. 先反向求累加和然后向右移一位，左端补0
            # rw_step_sum = [np.roll(np.cumsum(np.flipud(self.m_r_step[self.m_timestamp[w] + 1: len(self.m_r_step)])), 1)]
            # # 3. 然后两者相加
            # if len(rw_step_sum) != 0 and len(rw_step) != 0:
            #     rw_step_sum[0] = 0
            #     rw_step_sum += np.flipud(np.log(1 - np.exp(rw_step)))
            # # 4. 计算这个单词的累加eta
            # rw_eta = np.sum(np.exp(rw_step_sum)) * self.m_eta
            # # 5. 将结果更新到lambda中
            # self.m_lambda[:, w] += rw_eta
    ...
    def save_topics(self, filename):
        ...
        f = file(filename, "w") 
        betas = self.m_lambda + self.m_eta  # 这里是不是做了简化处理，只加一个eta？？？
        for beta in betas:
