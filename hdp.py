...
class hdp:
    ...
    def doc_e_step(self, count, doc, ss, Elogbeta, Elogsticks_1st, var_converge, fresh=False):
        ...
        while iter < max_iter and (converge < 0.0 or converge > var_converge):
            ...
            if converge < -0.000001:
                print "%s [batch_count = %d] warning, likelihood is decreasing! old_likelihood = %f new_likelihood = %f" % (getTime(), count, old_likelihood, likelihood)
        ...
        ss.m_var_sticks_ss += np.sum(var_phi, 0)  # 竖着加，加完最后是sigma(j,sigma(t, var_phi))
        ss.m_var_beta_ss[:, doc.words] += np.dot(var_phi.T, phi.T * doc.counts) # lambda更新公式的eta后面那一项
    ...
    def em_on_large_data(self, filename, var_converge, fresh):
        ...
        for line in file(filename):
            ...
            i += 1
            likelihood += self.doc_e_step(i, doc, ss, Elogbeta, Elogsticks_1st, var_converge, fresh=fresh)
    ...
    def hdp_to_lda(self):
        ...
        beta = self.m_beta / beta_sum[:, np.newaxis]  # 归一化lambda
    ...
    def em_with_testing(self, c, max_iter, var_converge, max_time, directory, c_test, split_ratio, seeded=True):
        ...
        while (max_iter == -1 or iter < max_iter) and totaltime < max_time:
            ...
            likelihood += (self.m_T - 1) * log_gamma
            dig_sum = sp.psi(np.sum(self.m_var_sticks, 0))
            likelihood += np.sum((np.array([1.0, self.m_gamma])[:, np.newaxis] - self.m_var_sticks) * (
