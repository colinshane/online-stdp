...
def run_hdp():
    ...
    if options.test_data_path_in_folds is not None:
        ...
        test_data_in_folds_filenames = glob(test_data_path_in_folds)  # 返回所有匹配文件路径列表
    ...
    while (options.max_iter == -1 or iter < options.max_iter) and total_time < options.max_time:
        ...
        if iter % 100 == 0:
            ...
            cPickle.dump(bhdp, file('%s/doc_count-%d.model' % (result_directory, total_doc_count), 'wb'), -1)
    ...
    cPickle.dump(bhdp, file('%s/final.model' % result_directory, 'wb'), -1)
